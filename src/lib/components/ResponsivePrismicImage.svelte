...
<figure class={figureClass}>
	<picture>
		...
		{#if image?.desktop?.url}
			<!-- Desktop: nutze den 16:9 Crop falls vorhanden -->
			<source media="(min-width: 641px)" srcset={image.desktop.url} sizes="100vw" />
		{/if}

		<!-- Fallback: generiert responsives srcset via PrismicImage -->
		<PrismicImage
			field={{
				url: image.url,
				alt: image.alt ?? null,
