<script lang="ts">
	...
	/**
	 * Transparenz erhalten: 'png' oder 'avif' verhindert dass Imgix auf JPEG konvertiert.
	 * Nur setzen wenn das Bild tatsächlich Transparenz hat.
	 */
	export let imgixFormat: 'auto' | 'png' | 'avif' | 'webp' | null = null;
...
<figure class={figureClass}>
	<picture>
		{#if image?.mobile?.url}
			...
			<source media="(max-width: 640px)" srcset={image.mobile.url} sizes="100vw" />
		{/if}
		...
		<PrismicImage
			...
			{loading}
			{decoding}
			imgixParams={imgixFormat ? { fm: imgixFormat } : undefined}
		/>
	</picture>
