<script lang="ts">
	...
	// Wohnung-Modus: Einzelne Zimmer buchbar (aus Kapazität-Tab)
	...
	// Zimmerauswahl-Checkboxen (nur im Einzelzimmer-Modus)
	...
	// Zimmer die im gewählten Zeitraum belegt sind
	...
	// Kalender-Ansicht: Im Ganze-Wohnung-Modus alle Perioden als zimmer:[] → alles blockiert
	...
	// Schlafzimmer-Auswahl: nur im Einzelzimmer-Modus
	...
	async function handleSubmit() {
		...
		// Ganze Wohnung → leere zimmerauswahl (blockiert alle); Einzelzimmer → ausgewählte Zimmer
...
>
	...
	{:else if ressource}
		...
		<!-- Buchungsart-Toggle (nur bei Wohnungen mit einzelbuchbaren Zimmern) -->
		{#if isWohnung}
			<div class="flex gap-2">
				<button
					type="button"
					on:click={() => switchBuchungsart('wohnung')}
					class="px-4 py-2 rounded text-sm font-medium transition-opacity"
		...
		<div class="grid md:grid-cols-2 gap-10">
			...
			<form on:submit|preventDefault={handleSubmit} class="flex flex-col gap-4">
				...
				<label class="flex flex-col gap-1 text-sm">
					<span>{$_('Name')} *</span>
					<input type="text" bind:value={name} required class="border px-3 py-2 rounded"
						style="background: transparent; color: inherit; border-color: {textColor}44;" />
				</label>
				<label class="flex flex-col gap-1 text-sm">
					<span>{$_('E-Mail')} *</span>
					<input type="email" bind:value={email} required class="border px-3 py-2 rounded"
						style="background: transparent; color: inherit; border-color: {textColor}44;" />
				</label>
				<label class="flex flex-col gap-1 text-sm">
					<span>{$_('Telefon')}</span>
					<input type="tel" bind:value={telefon} class="border px-3 py-2 rounded"
						style="background: transparent; color: inherit; border-color: {textColor}44;" />
				</label>
				<label class="flex flex-col gap-1 text-sm">
					<span>{$_('Nachricht')}</span>
					<textarea bind:value={nachricht} rows="3" class="border px-3 py-2 rounded resize-none"
						style="background: transparent; color: inherit; border-color: {textColor}44;"></textarea>
				</label>
