<script lang="ts">
	...
	$: textAlign = ausrichtung === 'Links' ? 'left' : ausrichtung === 'Rechts' ? 'right' : 'center';
		...
		<div class="grid gap-6" style="{gridColsStyle};">
			{#each items as item}
				<div
					...
					style="background-color: {p.karten_bg ||
						'color-mix(in srgb, currentColor 6%, transparent)'}; box-shadow: 0 2px 16px rgba(0,0,0,0.07);"
				>
			...
			<div style="display:flex; justify-content:{justifyStyle};">
				<div
					...
					style="background-color: {p.bg_color ||
						'var(--page-bg-color)'}; box-shadow: 0 4px 32px rgba(0,0,0,0.10); border: 1px solid color-mix(in srgb, currentColor 10%, transparent);"
				>
					<blockquote
						class="text-lg md:text-xl leading-relaxed mb-7"
		...
		{:else if stil === 'Modern'}
			...
			<div style="display:flex; justify-content:{justifyStyle};">
				<div class="max-w-2xl w-full pl-7 md:pl-9" style="border-left: 4px solid {accentColor};">
					<blockquote
						class="text-xl md:text-2xl font-medium leading-snug mb-5"
						style="text-align:{textAlign};"
					>
						<PrismicText field={p.quote} />
					</blockquote>
		...
		{:else if stil === 'Gross'}
			...
			<div style="display:flex; justify-content:{justifyStyle};">
				<figure class="max-w-5xl w-full" style="text-align:{textAlign};">
					<blockquote
						class="leading-tight mb-6 font-light"
