<script lang="ts">
	...
	$: pageTitle = data?.meta_title || data?.title || data.settings?.data?.meta_title || siteName;
	$: finalTitle = pageTitle === siteName ? siteName : `${pageTitle} | ${siteName}`;
	$: finalDesc = data?.meta_description || asText(data.settings?.data?.meta_description) || '';
	$: finalImage =
		data?.meta_image ||
		data?.page?.data?.meta_image?.url ||
		data.settings?.data?.meta_image?.url ||
		'';
	...
	$: currentUrl =
		typeof window !== 'undefined' ? `${cleanBaseUrl}${window.location.pathname}` : cleanBaseUrl;

	// --- 3. HREFLANG / SWITCHER LOGIK (Jetzt sauber sortiert) ---
	function getStaticAlternates(pathname: string) {
		const segments = pathname.split('/').filter(Boolean);
		const currentSlug =
			segments[0] === 'en-us' || segments[0] === 'de-ch' ? segments[1] : segments[0];

		for (const key in staticRoutes) {
			const mapping = staticRoutes[key];
			if (Object.values(mapping).includes(currentSlug)) {
				return [
					{ lang: 'de-ch', href: `${cleanBaseUrl}/${mapping['de-ch']}` },
					{ lang: 'en-us', href: `${cleanBaseUrl}/en-us/${mapping['en-us']}` },
					{ lang: 'x-default', href: `${cleanBaseUrl}/${mapping['de-ch']}` }
				];
			}
		}
		return [];
	...
	// Nur noch das hier für den Switcher:
	$: allAlternates = (() => {
		// A. Prismic Seiten nutzen die alternate_languages direkt
		if (data?.page?.alternate_languages) {
			return data.page.alternate_languages.map((alt: any) => ({
				lang: alt.lang,
				href: alt.url || (alt.lang === defaultLang ? `/${alt.uid}` : `/${alt.lang}/${alt.uid}`)
			}));
		}

		// B. Statische Seiten (Mapping)
	...
	// --- 5. FONTS ---
	$: adobeFontUrl = data.settings?.data?.adobe_font_id
		? `https://use.typekit.net/${data.settings.data.adobe_font_id}.css`
		: null;
	...
	// --- 6. STATES ---
	$: hasBannerOverlap =
		data?.page?.data?.slices?.find((s: any) => s.slice_type === 'hero')?.primary?.banner_overlap ===
		true;
	$: isLandingPage = data?.page?.data?.landing_page === true;

	onMount(() => {});
	afterNavigate(() => {});

	$: {
		console.log('1. Layout Alternates:', allAlternates);
	}
</script>
