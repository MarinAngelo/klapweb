<script lang="ts">
	...
	// Wir prüfen den Toggler aus den Prismic Settings
	// Wir erzwingen die Reaktivität hier
	$: showSwitcher = !!data.settings?.data?.show_language_switcher;
	$: console.log('Prismic Settings Raw:', data.settings?.data);
	$: console.log('Switcher Toggler Wert:', data.settings?.data?.show_language_switcher);
	...
	$: pageTitle =
		$page.data?.meta_title || $page.data?.title || data.settings?.data?.meta_title || siteName;
	$: finalTitle = pageTitle === siteName ? siteName : `${pageTitle} | ${siteName}`;
	$: finalDesc =
		$page.data?.meta_description || asText(data.settings?.data?.meta_description) || '';
	$: finalImage =
...
<svelte:head>
	...
	{#if seoAlternates.length > 0}
		<link
			rel="alternate"
			hreflang="x-default"
			href={cleanBaseUrl + (allAlternates.find((a) => a.lang === defaultLang)?.href || '/')}
		/>
	{/if}
