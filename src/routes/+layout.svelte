...
<script lang="ts">
    ...
    // Importiere convertNumber, falls es eine Hilfsfunktion ist, die Zahlen konvertiert
    // Stelle sicher, dass der Pfad korrekt ist
    import { convertNumber } from '$lib/utils';
    // onMount ist nicht mehr zwingend notwendig, da getCssVar eine window-Prüfung hat
    // und die Zuweisungen reaktiv sind. Ich lasse es hier, falls es an anderer Stelle gebraucht wird.
    ...
    $: {
        ...
        headerLinkHoverColor = data.prismicTheme.data?.header_link_hover_color || getCssVar('--header-link-hover-color');
        ...
        pageLinkActiveColor = data.prismicTheme.data?.page_link_active_color || getCssVar('--page-link-active-color');
        pageLinkVisitedColor = data.prismicTheme.data?.page_link_visited_color || getCssVar('--page-link-visited-color');
        ...
        pageLinkHoverColorBg = data.prismicTheme.data?.page_link_hover_color_bg || getCssVar('--page-link-hover-color-bg');
        pageLinkHoverColorText = data.prismicTheme.data?.page_link_hover_color_text || getCssVar('--page-link-hover-color-text');
