<script lang="ts">
	...
	function getGoogleFontsUrl(fontsArr: any[]): string | null {
		...
		const families = fontsArr
			...
			.map((f) => `family=${encodeURIComponent(f.data.name.replace(/\s+/g, '+'))}`);
		if (!families.length) return null;
		return `https://fonts.googleapis.com/css2?${families.join('&')}&display=swap`;
	}

	// Nur im Browser dynamisch Google Fonts einbinden!
	$: if (typeof window !== 'undefined' && Array.isArray(data?.fonts) && data.fonts.length > 0) {
		const url = getGoogleFontsUrl(data.fonts);
		if (url) {
			let link = document.querySelector('link[data-dynamic-google-fonts]');
			if (!link) {
				link = document.createElement('link');
				link.setAttribute('rel', 'stylesheet');
				link.setAttribute('data-dynamic-google-fonts', 'true');
