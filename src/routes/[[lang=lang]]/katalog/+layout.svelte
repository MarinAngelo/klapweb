<script lang="ts">
	...
	$: katalogBase = `${langPrefix}/katalog`;
...
<div>
	...
	>
		...
		<select
			...
			style="background-color: {bg}; color: {fg}; border: 1px solid {fgMuted}44; font-size: 16px;"
		>
			<option value="" disabled selected style="background-color: {fg}; color: {bg};"
				>{$_('Elemente auswählen')}</option
			>
			{#each filteredSlices as slice}
				<optgroup label={$_(slice.name)} style="background-color: {fg}; color: {bg};">
					{#each slice.variations as variation}
						{@const path = `${katalogBase}/${slice.dirName}/${variation.id}`}
						<option value={path} style="background-color: {fg}; color: {bg};"
							>{$_(slice.name)} – {$_(variation.name)}</option
						>
					{/each}
				</optgroup>
			{/each}
		...
		>
			...
			<div class="px-4 py-3" style="border-bottom: 1px solid {fgMuted}22;">
				<div class="flex flex-col gap-1">
					<span
						style="color: {fgMuted}; font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em;"
						>{$_('Plan')}</span
					>
					<select
						bind:value={selectedPlan}
					...
					>
						<option value="" style="background-color: {fg}; color: {bg};"
							>{$_('Paket auswählen')}</option
						>
						{#each data.plans as plan}
							<option value={plan.id} style="background-color: {fg}; color: {bg};"
								>{plan.label}</option
							>
						{/each}
					</select>
				...
				{#each filteredSlices as slice}
					<div class="mt-3">
						...
						{#each slice.variations as variation}
							...
							<a
								href="{katalogBase}/{slice.dirName}/{variation.id}"
