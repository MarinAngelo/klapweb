					...
					<thead>
						<tr style="border-bottom: 2px solid #e5e7eb; text-align: left;">
							{#each ['Status', 'Name', 'E-Mail', 'Buchung', 'Credit-Typ', 'Credits', 'Minuten', 'Kommentar', 'Angenommen am', 'Erledigt am', 'Aktionen'] as col}
								<th style="padding: 0.5rem 0.75rem; font-weight: 600; white-space: nowrap;">{col}</th>
							{/each}
						</tr>
					</thead>
					<tbody>
						{#each annahmen as a}
							<tr style="border-bottom: 1px solid #f3f4f6;">
								<td style={tdNowrap}>
									<span style="padding: 0.125rem 0.5rem; border-radius: 9999px; font-size: 0.75rem; background: {statusColors[a.status] ?? '#f3f4f6'};">
								...
								<td style="padding: 0.5rem 0.75rem; font-size: 0.75rem;">
									...
									<span style="font-family: monospace; word-break: break-all; user-select: all;">{a.buchungId}</span>
								</td>
								<td style={tdNowrap}>{a.creditTyp === 'fest' ? 'Fest' : 'Zeitbasiert'}</td>
								<td style={tdNowrap}>{a.credits != null ? a.credits : (a.creditBetrag != null ? a.creditBetrag : '–')}</td>
								<td style={tdNowrap}>{a.minuten != null ? `${a.minuten} min` : '–'}</td>
								<td style="padding: 0.5rem 0.75rem; max-width: 200px; font-size: 0.75rem; opacity: 0.8;">{a.kommentar ?? '–'}</td>
								<td style={tdNowrap}>{fmtDate(a.angenommenAt)}</td>
								<td style={tdNowrap}>{fmtDate(a.erledigtAt)}</td>
								<td style={tdStyle}>
									<div style="display: flex; flex-direction: column; gap: 0.375rem; min-width: 220px;">
										{#if a.status === 'angenommen'}
											<form method="POST" action="?/bestaetigen&secret={secret}" use:enhance>
												<input type="hidden" name="id" value={a.id} />
												<button type="submit" style="width: 100%; padding: 0.25rem 0.75rem; background: #2563eb; color: white; border: none; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;">
													Annahme bestätigen
												</button>
											</form>
										{/if}
										{#if a.status === 'eingereicht'}
											<form method="POST" action="?/freigeben&secret={secret}" use:enhance style="display: flex; flex-direction: column; gap: 0.25rem;">
												<input type="hidden" name="id" value={a.id} />
												<label for="credits-{a.id}" style="font-size: 0.7rem; color: #6b7280;">Credits (anpassen)</label>
												<input
													id="credits-{a.id}"
													type="number"
													name="credits"
												...
												></textarea>
												<button type="submit" style="width: 100%; padding: 0.25rem 0.75rem; background: #16a34a; color: white; border: none; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;">
													✓ Freigeben &amp; Erledigt
												</button>
											</form>
										{/if}
										{#if !['eingereicht', 'erledigt'].includes(a.status)}
											<form method="POST" action="?/erledigt&secret={secret}" use:enhance style="display: flex; flex-direction: column; gap: 0.25rem;">
												<input type="hidden" name="id" value={a.id} />
										...
										{#if a.status !== 'angenommen'}
											<form method="POST" action="?/zuruecksetzen&secret={secret}" use:enhance>
												<input type="hidden" name="id" value={a.id} />
												<button type="submit" style="width: 100%; padding: 0.25rem 0.75rem; background: #6b7280; color: white; border: none; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;">
													↺ Zurücksetzen
												</button>
											</form>
										{/if}
										<form method="POST" action="?/loeschen&secret={secret}" use:enhance on:submit={(e) => confirmLoeschen(e, a.name)}>
											<input type="hidden" name="id" value={a.id} />
											<button type="submit" style="width: 100%; padding: 0.25rem 0.75rem; background: #dc2626; color: white; border: none; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;">
												Löschen
											</button>
										</form>
									</div>
