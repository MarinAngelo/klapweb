...
export const GET: RequestHandler = async ({ url }) => {
	...
	const type = typeParam === 'abreise' ? 'abreise' : typeParam === 'nach_ankunft' ? 'nach_ankunft' : 'ankunft';
	...
	const defaultDate = type === 'abreise' ? dateInDays(1) : type === 'nach_ankunft' ? dateInDays(-1) : dateInDays(2);
	...
	const upcoming = type === 'abreise'
		? allBookings.filter((b) => b.bis === targetDateStr && b.email && !b.abreiseReminderSent)
		: type === 'nach_ankunft'
			? allBookings.filter((b) => b.von === targetDateStr && b.email && !b.nachAnkunftReminderSent)
			: allBookings.filter((b) => b.von === targetDateStr && b.email && !b.reminderSent);

	const emptyMsg = type === 'abreise' ? 'Keine Abreisen gefunden'
		: type === 'nach_ankunft' ? 'Keine gestrigen Ankünfte gefunden'
	...
	for (const buchung of upcoming) {
		try {
			...
			const textField = type === 'abreise'
				? (d.abreise_text as any[] | undefined)
				: type === 'nach_ankunft'
					? (d.nach_ankunft_text as any[] | undefined)
					: (d.reminder_text as any[] | undefined);

			const fieldName = type === 'abreise' ? 'abreise_text' : type === 'nach_ankunft' ? 'nach_ankunft_text' : 'reminder_text';
			if (!textField?.length) {
				results.push({ id: buchung.id, status: 'skip', reason: `Kein ${fieldName}` });
				continue;
			}

			const betreff = type === 'abreise'
				? ((d.abreise_betreff as string)?.trim() || `Ihre Abreise morgen: ${d.name ?? buchung.ressourceUid}`)
				: type === 'nach_ankunft'
					? ((d.nach_ankunft_betreff as string)?.trim() || `Willkommen: ${d.name ?? buchung.ressourceUid}`)
					: ((d.reminder_betreff as string)?.trim() || `Ihre Anreise morgen: ${d.name ?? buchung.ressourceUid}`);
			...
			const flagKey = type === 'abreise' ? 'abreiseReminderSent' : type === 'nach_ankunft' ? 'nachAnkunftReminderSent' : 'reminderSent';
			...
			const typeLabel = type === 'abreise' ? 'Abreise' : type === 'nach_ankunft' ? 'Nach-Ankunft' : 'Ankunfts';
